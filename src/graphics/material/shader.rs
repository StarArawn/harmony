    ...
    pub fn new(device: &wgpu::Device, path: String, file_name: String) -> Self {
        ...
        let vertex = {
            let spirv = compiler.compile_into_spirv(
                &vert_contents, shaderc::ShaderKind::Vertex,
                "vertex.glsl", "main", Some(&options)).unwrap();
            device.create_shader_module(&spirv.as_binary())
        };

        let fragment = {
            let spirv = compiler.compile_into_spirv(
                &frag_contents, shaderc::ShaderKind::Fragment,
                "pixel.glsl", "main", Some(&options)).unwrap();
            device.create_shader_module(spirv.as_binary())
        };
