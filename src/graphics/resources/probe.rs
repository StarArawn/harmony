...
use super::{GPUResourceManager, RenderTarget, BindGroup};
use crate::{graphics::{pipeline_manager::PipelineManager, RenderGraph}, scene::components::CameraData, AssetManager};
...
impl Probe {
    pub(crate) fn new(id: u32, position: Vec3, resources: &Resources, quality: ProbeQuality, format: ProbeFormat) -> Self {
        let sample_offset = 0;
        ...
        let irradiance_target = RenderTarget::new(&device, irradiance_resoultion as f32, irradiance_resoultion as f32, 6, 1, wgpu_format, wgpu::TextureUsage::SAMPLED | wgpu::TextureUsage::COPY_DST);
        ...
        let bind_group = BindGroup::new(3, device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            bindings: &[
                wgpu::Binding {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&irradiance_target.texture_view),
                },
                wgpu::Binding {
                    binding: 1,
    ...
    pub(crate) fn render_scene(&mut self, resources: &mut Resources, scene: &mut crate::scene::Scene) {
        ...
        
        {
            ...
            for i in 0..6 {
                // TODO: cache views?
                let view = self.probe_cube.texture.create_view(&wgpu::TextureViewDescriptor {
        ...
        let mut probe_cube = {
            let device = resources.get::<wgpu::Device>().unwrap();
            RenderTarget::new(&device, probe_resoultion as f32, probe_resoultion as f32, 6, 1, self.format.into(), wgpu::TextureUsage::SAMPLED | wgpu::TextureUsage::COPY_SRC | wgpu::TextureUsage::OUTPUT_ATTACHMENT)
        };
        probe_cube.texture = crate::graphics::pipelines::mipmap::create(resources, &self.probe_cube.texture, self.format.into(), wgpu::TextureDimension::D2, probe_resoultion, probe_resoultion, 6);
        probe_cube.texture_view = probe_cube.texture.create_view(&wgpu::TextureViewDescriptor {
    ...
    pub(crate) fn render_brdf(&mut self, resources: &mut Resources, _scene: &mut crate::scene::Scene) {
    ...
    fn render_irradiance(&mut self, resources: &Resources) {
        ...
        if graph_node.is_none() {
            let pipeline_desc = crate::graphics::pipelines::irradiance2::IrradiancePipelineDesc::default();
            render_graph.add(&asset_manager, &device, &sc_desc, &mut resource_manager, "irradiance2", pipeline_desc, vec![], false, None, false);
        }

        let pipeline = &render_graph.get("irradiance2").pipeline;

        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("irradiance") });

        let global_bind_group = resource_manager.get_bind_group_layout("irradiance2").unwrap();
        ...
        let uniform = ProbeUniform {
            data: Vec4::new(self.sample_offset as f32, self.samples_per_frame as f32, self.sample_count as f32, self.scale),
            data2: Vec4::new(self.irradiance_resoultion as f32, self.irradiance_resoultion as f32, 0.0, 0.0),
        };
    ...
    fn render_specular(&mut self, resources: &Resources) {
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("specular") });

        let global_bind_group = resource_manager.get_bind_group_layout("specular_globals").unwrap();
        ...
        for mip_id in 0..mip_levels {
            let uniform = ProbeUniform {
                data: Vec4::new(self.sample_offset as f32, self.samples_per_frame as f32, self.sample_count as f32, self.scale),
                data2: Vec4::new(self.specular_resoultion as f32, self.specular_resoultion as f32, roughness, mip_id as f32),
            };
            let uniform_buf = device.create_buffer_with_data(bytemuck::bytes_of(&uniform), wgpu::BufferUsage::UNIFORM | wgpu::BufferUsage::COPY_SRC);
