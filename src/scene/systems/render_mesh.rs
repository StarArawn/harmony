...
use crate::{
    ...
    scene::components::{ CameraData, Mesh, Transform, transform::LocalUniform }
};
...
impl<'a> System<'a> for RenderMesh<'a> {
    ...
    fn run(&mut self, (camera_data, meshes, materials, mut transforms): Self::SystemData) {
...

        {
            ...
            for (transform, slot) in (&mut transforms).join()
        ...
        for asset_material in asset_materials {
            ...
            match asset_material {
                Material::Unlit(unlit_material) => {
                    render_pass.set_bind_group(2, &unlit_material.bind_group_data.as_ref().unwrap().bind_group, &[]);
                    for (mesh, _, transform) in joined_data.filter(|(_, material, _)| material.index == unlit_material.index) {
